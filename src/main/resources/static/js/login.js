...
//			alert($loginWindowForm.serialize());
			...
			$.ajax({
				...
				success: function(data) {
					...
					if (data == "success") {
                        ...
                        window.location.href="/user/reSendVerifyMail?username="+loginWindowMemberUsername;
					}else if(data =="admin"){
						alert("登录失败，原因：您是管理员！！！");
					}else if(data == "frozen"){
		...
		function memberRegister(){
            ...
            var $registerWindowMemberRealName = document.getElementById("realName").value;
            ...
            var $registerWindowMemberCheck = document.getElementsByName("agree");
...
//			alert($registerWindowForm.serialize());
            ...
            if ($registerWindowMemberRealName== "") {
                alert("请输入您的真实姓名")
                return false;
            }
            // var reg = new RegExp("[\\u4E00-\\u9FFF]+","g");
            if (!/^[(\u4e00-\u9fa5)]+$/.test($registerWindowMemberRealName)) {
...
//				$registerWindowReMemberPassword.focus();
            ...
            if (!$registerWindowMemberCheck[0].checked) {
//				$registerWindowReMemberPassword.focus();
                alert("请选择阅读并同意!");
                return false;
            }
				...
				success: function(data) {
					if (data == "true") {
						...
						$.ajax({
							...
							success: function(data) {
								if (data == "success") {
									window.location.href="/user/sendVerifyMailSuccess";
								}else if(data == "send_fail"){
                                    window.location.href="/user/sendVerifyMailFail?username="+$registerWindowMemberUsername;
								}else{
									alert("注册失败");
								}
